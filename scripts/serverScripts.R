...
densityPlot <- function(input, data, data2, data3){
  ...
  renderPlot({
    ...
    variantXintercept <- variantXintercept[variantXintercept$p_variant %in% c(data2[input$promoterAssayData_rows_selected]$p_variant, functionVars),]
...
heatmapPlot <- function(input, data){
  ...
  renderPlot({
    ...
    data <- data[data$variable %in% c(input$promoterCheckGroup2_kato, input$promoterCheckGroup2_giacomelli, input$promoterCheckGroup2_hahn, input$promoterCheckGroup2_boettcher),]
